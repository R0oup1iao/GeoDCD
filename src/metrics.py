...
def count_accuracy(B_true, B_prob, ignore_diag=True):
    ...
    B_true: true adjacency matrix (N, N) (binary)
    B_prob: predicted weight matrix (N, N) (continuous)
    Note: diagonal elements are ignored by default.
    """
    n = B_true.shape[0]
    if ignore_diag:
    ...
    if len(np.unique(true_flat)) < 2:
        # If Ground Truth is all 0 or all 1, cannot compute AUC, return 0.5 or 0
        auroc = 0.5
        auprc = 0.0
    else:
    ...
    # Sample 100 thresholds for search
