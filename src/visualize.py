...
def create_dynamic_gif(dynamic_adj, save_path, fps=10):
    """
    Generate dynamic causal graph GIF
    dynamic_adj: (N, N, T) - note the last dimension is time
    save_path: output.gif
    """
    # dynamic_adj shape: (Source, Target, Time) or (Target, Source, Time)?
    # GeoDCD output strengths is [Source, Target, Time].
    # Conventionally Heatmap is Row=y(Target), Col=x(Source) or vice versa
    # We uniformly display: Y-axis as Target, X-axis as Source
    
    N, _, T = dynamic_adj.shape
    
    fig, ax = plt.subplots(figsize=(8, 6))
    
    # Initial frame (for Colorbar Range, we take global maximum)
    vmax = np.percentile(dynamic_adj, 99) # Ignore extreme outliers
    vmin = 0
